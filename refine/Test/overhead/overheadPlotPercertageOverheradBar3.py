...
def create_comparison_plot(data_no_tee, data_tee, data_overhead_abs, data_overhead_perc,
    ...
    # Regola automaticamente i limiti dell'asse y per dare spazio alle etichette
    max_value = max(max(data_no_tee), max(data_tee), max(data_overhead_abs))
    plt.ylim(0, max_value * 1.07)  # Aggiungi 15% di spazio in alto

    plt.tick_params(axis='x', labelsize=18)
    plt.tick_params(axis='y', labelsize=18)
    plt.tight_layout()
    plt.savefig(filename, bbox_inches='tight')  # bbox_inches='tight' aiuta con i margini
    plt.show()


# Crea i grafici
create_comparison_plot(ram_no_tee, ram_tee, overhead_ram_abs, overhead_ram_perc,
                       "Average Memory Usage [Mb]", "./output/ramOverheadperc3.pdf", 0.2, 0.2)
create_comparison_plot(ram_no_tee, ram_tee, overhead_ram_abs, overhead_ram_perc,
                       "Average Memory Usage [Mb]", "./output/ramOverheadperc3.png", 0.2, 0.2)

create_comparison_plot(time_no_tee, time_tee, overhead_time_abs, overhead_time_perc,
                       "Average Duration [ms]", "./output/timeOverheadperc3.pdf", 50, 50)
create_comparison_plot(time_no_tee, time_tee, overhead_time_abs, overhead_time_perc,
                       "Average Duration [ms]", "./output/timeOverheadperc3.png", 50, 50)