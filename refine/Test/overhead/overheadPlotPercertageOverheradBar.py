...
# Calcolo scarto percentuale
def percent_overhead(base, tee):
    return [( (t - b) / b * 100 ) if b != 0 else 0 for b, t in zip(base, tee)]

overhead_ram = percent_overhead(ram_no_tee, ram_tee)
overhead_time = percent_overhead(time_no_tee, time_tee)

# Parametri per barre affiancate
x = np.arange(len(labels_display))
width = 0.2

def add_bar_labels(bars, offset=0.2, percent=False):
